...
const LawyerListModal: React.FC<LawyerListModalProps> = ({
  ...
  const handleConnectClick = (e: React.MouseEvent, lawyerId: string) => {
    e.stopPropagation();
    console.log(`Connect clicked for lawyer: ${lawyerId}`);
  };
              ...
              <div className="space-y-4">
                {lawyers.map(lawyer => (
                  <LawyerPreviewCard 
                    ...
                    onConnectClick={(e) => handleConnectClick(e, lawyer.id)}
                  />
                ))}
              </div>
            ) : (
